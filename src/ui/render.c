...
void render_Circle(int size_px, float x_center, float y_center, Color color) {
    ...
    for (int i = n; i > 0; i--, y++){
        glBegin(GL_LINES);
            glVertex2f(x_center - (size_px/2.0 - i) / (float) window_width(), y_center + (size_px/2.0 - y) / (float) window_height());
            glVertex2f(x_center + (size_px/2.0 - i) / (float) window_width(), y_center + (size_px/2.0 - y) / (float) window_height());
        glEnd();
    }
    for (int i = 0; i < size_px - 2*n; i++, y++) {
        glBegin(GL_LINES);
            glVertex2f(x_center - size_px/2.0 / (float) window_width(), y_center + (size_px/2.0 - y) / (float) window_height());
            glVertex2f(x_center + size_px/2.0 / (float) window_width(), y_center + (size_px/2.0 - y) / (float) window_height());
        glEnd();
    }
    for (int i = 1; i <= n; i++, y++){
...
void render_Pacman_mouth(int size_px, float x_center, float y_center, Color color, Direction direction) {
    ...
    if (direction == BOTTOM) {
        ...
        while (i <= 2*n) {   
            glBegin(GL_LINES);
                glVertex2f(x_center + (size_px/2.0 - y) / (float) window_width(), y_center - (size_px/2.0 - i) / (float) window_height());
                glVertex2f(x_center + (size_px/2.0 - y) / (float) window_width(), y_center - (size_px/2.0) / (float) window_height());
            glEnd();
            i += 2;
            y++;
        }
...
void render_Eyes(int size_px, float x_center, float y_center, enum Direction direction){
    render_Circle(size_px*0.3, x_center - (size_px / 4) / (float) window_width(), y_center + (size_px*0.2) / (float) window_height(), WHITE);// EYES
    render_Circle(size_px*0.3, x_center + (size_px / 4) / (float) window_width(), y_center + (size_px*0.2) / (float) window_height(), WHITE);// EYES

    if (direction == BOTTOM){
        render_Circle(0.15*size_px, x_center - (size_px / 4) / (float) window_width(), y_center + (size_px*0.1) / (float) window_height(), BLACK);
        render_Circle(0.15*size_px, x_center + (size_px / 4) / (float) window_width(), y_center + (size_px*0.1) / (float) window_height(), BLACK);
    }
    if (direction == TOP){
        render_Circle(0.15*size_px, x_center - (size_px / 4) / (float) window_width(), y_center + (size_px*0.3) / (float) window_height(), BLACK);
        render_Circle(0.15*size_px, x_center + (size_px / 4) / (float) window_width(), y_center + (size_px*0.3) / (float) window_height(), BLACK);
    }
    if (direction == RIGHT){
        render_Circle(0.15*size_px, x_center + (size_px * 0.1+ size_px / 4) / (float) window_width(), y_center + (size_px*0.2) / (float) window_height(), BLACK);
        render_Circle(0.15*size_px, x_center - (size_px * 0.15) / (float) window_width(), y_center + (size_px*0.2) / (float) window_height(), BLACK);
    }
    if (direction == LEFT){
        render_Circle(0.15*size_px, x_center - (size_px * 0.1+ size_px / 4) / (float) window_width(), y_center + (size_px*0.2) / (float) window_height(), BLACK);
        render_Circle(0.15*size_px, x_center + (size_px * 0.15) / (float) window_width(), y_center + (size_px*0.2) / (float) window_height(), BLACK);
...
void render_Ghost(int size_px, float x_center, float y_center, Color color, Direction direction){
    ...
    for (int i = size_px/5; i > 0; i--){
        glBegin(GL_LINES);
            glVertex2f(x_center - (size_px / 2 - i) / (float) window_width(), y / (float) window_height());
            glVertex2f(x_center + (size_px / 2 - i) / (float) window_width(), y / (float) window_height());
        glEnd();
        y--;
    }
    for (y ; y > (int)(y_center * window_height() - size_px*0.6 / 2); y--){
        glBegin(GL_LINES);
            glVertex2f(x_center - (size_px / 2) / (float) window_width(), y / (float) window_height());
            glVertex2f(x_center + (size_px / 2) / (float) window_width(), y / (float) window_height());
        glEnd();
    }
    glBegin(GL_TRIANGLES);
        glVertex2f(x_center - (size_px / 2 )/ (float) window_width(), y_center - (0.3*size_px) / (float) window_height());
        glVertex2f(x_center - (size_px / 2 - size_px / 4) / (float) window_width(),y_center - (0.3*size_px) / (float) window_height());
        glVertex2f(x_center - (size_px / 2) / (float) window_width(), y_center - (size_px /2) / (float) window_height());

        glVertex2f(x_center - (1.3 * size_px / 8 )/ (float) window_width(), y_center - (0.3*size_px) / (float) window_height());
        glVertex2f(x_center + (1.3 *size_px / 8 ) / (float) window_width(),y_center - (0.3*size_px) / (float) window_height());
...
void render_Food(int size_px, float x_center, float y_center, Color color, enum Food food){
    ...
    if (food == SMALL){
        glBegin(GL_POLYGON);
            glVertex2f(x_center - (size_px/7)/(float) window_width(), y_center + (size_px/7)/(float) window_height());
            glVertex2f(x_center + (size_px/7)/(float) window_width(), y_center + (size_px/7)/(float) window_height());
            glVertex2f(x_center + (size_px/7)/(float) window_width(), y_center - (size_px/7)/(float) window_height());
            glVertex2f(x_center - (size_px/7)/(float) window_width(), y_center - (size_px/7)/(float) window_height());
        glEnd();
        ...
        render_Circle(size_px*0.3, x_center + (size_px/5)/(float) window_width(), y_center + (size_px/5)/(float) window_height(), color);
        render_Circle(size_px*0.3, x_center + (size_px/5)/(float) window_width(), y_center - (size_px/5)/(float) window_height(), color);
        render_Circle(size_px*0.3, x_center - (size_px/5)/(float) window_width(), y_center - (size_px/5)/(float) window_height(), color);
        render_Circle(size_px*0.3, x_center - (size_px/5)/(float) window_width(), y_center + (size_px/5)/(float) window_height(), color);
    }
