...
void draw_right_part(Game *game) {
    ...
    render_string("PACMAN GAME!", game->window_x + alpha_relative_x + FONT_WIDTH, 1 - alpha_relative_y - FONT_HEIGHT_UPPER_CASE, FONT, WHITE);
}
...
void draw_background(Game *game) {
    ...
    glBegin(GL_POLYGON);
        ...
        glVertex2f(game->window_x + alpha_relative_x, game->window_y - alpha_relative_y);
...
void draw_Pacman(Game *game, float x, float y) {
    ...
    if (mouth_opened) {
        render_Pacman_mouth(game->px_creature, x, y, GAME_BG, game->pacman->direction);
    }
}

void draw_Ghost(Game *game, float x, float y, int ghost_id){
    render_Ghost(game->px_creature, x, y, RED, game->ghosts[ghost_id]->direction);
}
...
void draw_game(Game *game) {
    ...
    for (int x=0; x<game->width; x++) {
        for (int y=0; y<game->height; y++) {
            ...
            float x_window;
            float y_window;
            map_xy_to_window_xy(game->alpha, x+1, y+1, &x_window, &y_window);
            switch (c.object)
            {
            case Nothing:
                break;
